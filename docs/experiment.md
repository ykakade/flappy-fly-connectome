...
> This is a **connectome-derived controller experiment**, not a real fly playing a game or a biologically validated brain simulation. MaleCNS supplies structural wiring. Tanh dynamics, game-state adapters, PPO and the game artwork are engineering additions. Inputs are structured state, **not pixels**. No consciousness, biological plasticity or full-brain emulation is claimed.

## Run the saved browser demo

The website now opens in a separate classic Play mode. The saved controllers, training results, and physics described below apply to the computer modes. The browser draws both games with Flappy Bird sprites. The experiment physics and saved models have not changed.
...
- **Play:** space/up arrow, mouse click or tap to flap; restart button or R to reset. Best scores are device-local. Play and the computer modes have separate records.
- **Watch:** online inference from a saved checkpoint; actions sampled from its flap probability.
- **You vs. computer:** yellow human and blue computer birds use independent environments with identical obstacle seeds. Both scores are shown.
- **Training:** untrained, early, mid and validation-selected best checkpoints on the same seed; validation curves for all architectures.
- **Change the wiring:** selected intervention versus original controller, same obstacle and action-sampling seed, with scores, score difference and probability traces.
...
**Local metric viewer:** serve `dist` and open Training. `progress.json` records every update; validation is every fourth update. Rerun evaluation to refresh `dist/training.json` after additional training. This is the equivalent local viewer; TensorBoard is not required.
...
- The browser uses Flappy Bird sprites and sounds from Samuel Custodio’s asset collection. See the included asset license. Wing animation is cosmetic.
